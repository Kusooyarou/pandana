...
class Accessibility {
 public:
    ...
    void initializeCategory(const double maxdist, const int maxitems, string category, vector<int> node_idx);
    ...
    void initializeAccVar(string category, vector<int> node_idx,
    ...
    vector<vector<pair<int, float>>> Range(vector<int> srcnodes, float radius, 
                                            int graphno, vector<int> ext_ids);

    // shortest path between two points
    vector<int> Route(int src, int tgt, int graphno = 0);

    // shortest path between list of origins and destinations
    vector<vector<int>> Routes(vector<int> sources, vector<int> targets,  
                               int graphno = 0);
    ...
    vector<vector<int>> RoutesWithTripIds(vector<int> sources, vector<int> targets,  
                                          int graphno = 0);
    ...
    vector<double> Distances(vector<int> sources, vector<int> targets,  
                             int graphno = 0);
